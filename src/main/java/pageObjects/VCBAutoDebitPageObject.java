...
public class VCBAutoDebitPageObject extends AbstractPage {
	...
	public String inputCustomerId(List<String> listCustomer) {
		...
		for (String customer : listCustomer) {
			inputToDynamicEditviewByLinearlayoutId(driver, customer, "com.VCB:id/linEdtContent");

			if (isDynamicCheckBoxByIdChecked(driver, "com.VCB:id/checkBox") == false) {
				clickDynamicCheckBox(driver, "com.VCB:id/checkBox");
			}
		...
		if (!check) {
			throw new SkipException("Khong ma khach hang nao thoa man");
		}
		return customerID;
	}
}
