...
public class Saving_Target_Flow extends Base {
	...
	public void TC_01_MucTieuTietKiem_Flow() {
		...
		verifyEquals(savingTarget.getDynamicTextDetailByIDOrPopup(driver, "com.VCB:id/tvTitleBar"), SavingTarget_Data.VALIDATE.SAVING_TARGET_TITLE);
		...
		savingTarget.inputToDynamicInputBox(driver, SavingTarget_Data.DATA.SAVING_MONEY_VND, SavingTarget_Data.VALIDATE.SAVING_MONEY_TEXT);
		...
		savingTarget.clickToDynamicButtonLinkOrLinkText(driver, SavingTarget_Data.DATA.TERM_RATE_12_MONTHS);
		
		log.info("TC_01_Step_08: Chon thoi gian can tich luy");
		savingTarget.clickToDynamicButtonLinkOrLinkText(driver, SavingTarget_Data.VALIDATE.ACCUMULATION_TIME_DRODDOWN_TEXT);
		...
		verifyEquals(savingTarget.getDynamicTextDetailByIDOrPopup(driver, "com.VCB:id/tvTitleBar"), SavingTarget_Data.VALIDATE.SAVING_TARGET_RESULT_TITLE);
		
		log.info("TC_01_Step_11: Xac nhan hien thi label Luu y");
		verifyTrue(savingTarget.isDynamicMessageAndLabelTextDisplayed(driver, SavingTarget_Data.VALIDATE.SAVING_TARGET_RESULT_TITLE_HEAD));
		
		log.info("TC_01_Step_12: Xac nhan hien thi dung so tien gui dinh ky");
		verifyEquals(savingTarget.getDynamicTextByLabel(driver, "Số tiền gửi định kỳ"), addCommasToLong(SavingTarget_Data.DATA.SAVING_MONEY_VND)+ " VND");
		...
		verifyEquals(savingTarget.getDynamicTextByLabel(driver, "Số tiền góp hàng tháng"), 
				addCommasToDouble(convertAvailableBalanceCurrentcyToDouble(SavingTarget_Data.DATA.SAVING_MONEY_VND)/12+"")+ " VND");
		...
		double b = convertMoneyToDouble(getSplitStringIndex(SavingTarget_Data.DATA.TERM_RATE_12_MONTHS,"/",1),"VND")/100;
		interestMoney = a*b;
