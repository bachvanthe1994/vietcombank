...
public class Login_Validation extends Base {
	...
	public void TC_01_KiemTraMacDinhTextBoxNhapMatKhau(){
		...
		verifyTrue(login.isDynamicMessageAndLabelTextDisplayed(LogIn_Data.Message.WELCOME_MESSAGE));
		
		log.info("TC_01_Step_02: Kiem tra textbox dien so dien thoai hien thi");
		verifyTrue(login.isDynamicTextInInputBoxDisPlayed(LogIn_Data.UI.PHONE_NUMBER));
		
		log.info("TC_01_Step_03: Kiem tra Dang ky truc tuyen hien thi");
		verifyTrue(login.isDynamicMessageAndLabelTextDisplayed(LogIn_Data.UI.ONLINE_REGISTER));
		
		log.info("TC_01_Step_04: Kiem tra Đăng ký hiển thị");
		verifyTrue(login.isDynamicMessageAndLabelTextDisplayed(LogIn_Data.UI.REGISTER));
		
		log.info("TC_01_Step_05: Dien so dien thoai");
		login.inputToDynamicInputBox(LogIn_Data.Login_Account.PHONE, LogIn_Data.UI.PHONE_NUMBER);
		...
		verifyTrue(login.isDynamicMessageAndLabelTextDisplayed(LogIn_Data.Message.INPUT_PASSWORD_MESSAGE));
		
		log.info("TC_01_Step_08: Kiểm tra title nhap mat khau hien thi");
		verifyTrue(login.isDynamicTextInInputBoxDisPlayed(LogIn_Data.UI.PASSWORD_LABEL));
		
		log.info("TC_01_Step_09: Kiem tra quen mat khau hien thi");
		verifyTrue(login.isDynamicMessageAndLabelTextDisplayed(LogIn_Data.UI.FORGOT_PASSWORD));
	...
	public void TC_02_KiemTraKhiNhapKyTuTrong(){
		...
		login.inputToDynamicInputBox(LogIn_Data.Login_Account.NULL, LogIn_Data.UI.PASSWORD_LABEL);
		
		log.info("TC_02_Step_02: Click tiep tuc");
		login.clickToDynamicButton("Tiếp tục");
		
		log.info("TC_02_Step_03: Kiem tra pop-up yeu cau nhap mat khau hien thi");
		verifyTrue(login.isDynamicMessageAndLabelTextDisplayed(LogIn_Data.Message.EMPTY_MESSAGE));
		
		log.info("TC_02_Step_04: Click Dong");
		login.clickToDynamicButton("Đóng");
	}
	@Test
	public void TC_03_KiemTraLoaiKyTuNhap(){
		
		log.info("TC_03_Step_01: Dien mat khau khong dung dinh dang");
		login.inputToDynamicInputBox(LogIn_Data.Login_Account.INVALID_PASSWORD, LogIn_Data.UI.PASSWORD_LABEL);
		
		log.info("TC_03_Step_02: Kiem tra mat khau do khong duoc nhap");
		verifyTrue(login.getTextDynamicTextInInputBox(LogIn_Data.UI.PASSWORD_LABEL).equals(LogIn_Data.UI.PASSWORD_LABEL));
	...
	public void TC_04_KiemTraSoLuongKyTuToiDa(){
		...
		login.inputToDynamicInputBox(LogIn_Data.Login_Account.PASSWORD_GREATER_THAN_20, LogIn_Data.UI.PASSWORD_LABEL);
		
		log.info("TC_04_Step_04: Kiem tra mat khau do khong duoc nhap");
		verifyTrue(!login.getTextDynamicTextInInputBox(LogIn_Data.UI.PASSWORD_LABEL).equals(LogIn_Data.Login_Account.PASSWORD_GREATER_THAN_20));
	...
	public void TC_05_NhapSaiMatKhau(){
		...
		login.inputToDynamicInputBox(LogIn_Data.Login_Account.WRONG_PASSWORD, LogIn_Data.UI.PASSWORD_LABEL);
		
		log.info("TC_05_Step_04: Click Tiep Tuc");
		login.clickToDynamicButton("Tiếp tục");
		
		log.info("TC_05_Step_05:Kiem tra pop up sai mat khau hien thi");
		verifyTrue(login.isDynamicMessageAndLabelTextDisplayed(LogIn_Data.Message.WRONG_PASSWORD_MESSAGE));
	...
	public void TC_06_KiemTraTaiKhoanBiKhoa(){
		...
		login.inputToDynamicLogInTextBox(LogIn_Data.Login_Account.LOCKED_PHONE, "Tiếp tục");
		
		log.info("TC_06_Step_03: Click Tiep tuc");
		login.clickToDynamicButton("Tiếp tục");
		
		log.info("TC_06_Step_04: Kiem tra pop-up tai khoan da bi khoa hien thi");
		verifyTrue(login.isDynamicMessageAndLabelTextDisplayed(LogIn_Data.Message.LOCKED_ACCOUNT_MESSAGE));
