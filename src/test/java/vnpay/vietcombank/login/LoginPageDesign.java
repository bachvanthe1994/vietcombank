...
//		login.verifyIconChangeLanguage();
		...
		verifyTrue(login.isDynamicMessageAndLabelTextDisplayed(LogIn_Data.UI.FIND_ATM));
	...
	public void TC_03_KiemTraButtonTiepTuc(){
		...
		verifyTrue(login.isDynamicMessageAndLabelTextDisplayed(LogIn_Data.Message.PHONE_NUMBER_NOT_EMPTY));
		verifyTrue(login.isDynamicButtonDisplayed(LogIn_Data.UI.CLOSE_BUTTON));
		
		log.info("TC_03_1_3 Nhan nut Dong");
		login.clickToDynamicButton(LogIn_Data.UI.CLOSE_BUTTON);
		...
		verifyTrue(login.isDynamicMessageAndLabelTextDisplayed(LogIn_Data.Message.PHONE_NUMBER_INVALID));
		verifyTrue(login.isDynamicButtonDisplayed(LogIn_Data.UI.CLOSE_BUTTON));
		...
		verifyTrue(login.isDynamicMessageAndLabelTextDisplayed(LogIn_Data.Message.PHONE_NUMBER_VALID));
		verifyTrue(login.isDynamicButtonDisplayed(LogIn_Data.UI.CANCEL_BUTTON));
		verifyTrue(login.isDynamicButtonDisplayed(LogIn_Data.UI.AGREE_BUTTON));
		
		log.info("TC_03_4_3 Nhan nut Huy");
		login.clickToDynamicButton(LogIn_Data.UI.CANCEL_BUTTON);
		...
		verifyTrue(login.isDynamicMessageAndLabelTextDisplayed(LogIn_Data.Message.PHONE_NOT_REGISTER));
		verifyTrue(login.isDynamicButtonDisplayed(LogIn_Data.UI.REGISTER_NOW_BUTTON));
		verifyTrue(login.isDynamicMessageAndLabelTextDisplayed(LogIn_Data.UI.CONTINUE_USE_SERVICE_BUTTON));
		
		log.info("TC_03_5_3 Nhan nut Tiep tuc su dung dich vu");
		login.clickToDynamicButtonLinkOrLinkText(LogIn_Data.UI.CONTINUE_USE_SERVICE_BUTTON);
